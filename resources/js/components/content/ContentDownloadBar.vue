...
onMounted(() => {
    getActiveDownloads()
    emitter.on('download-output', handelDownloadOutput);
    emitter.on('file-added-to-queue', handelFileAddedToQueue);
...
const handelFileAddedToQueue = (payload: downloadOutput) => {
    downloadInProgress.value = true
    addOrUpdateDownload(payload.downloadOutput)
}

const handelDownloadOutput = (payload: downloadOutput) => {
    addOrUpdateDownload(payload.downloadOutput)
    downloadInProgress.value = true
    if (payload.downloadOutput.percent === 100) {
        $toast.open({
            message: 'File ' + payload.downloadOutput.content.title.toLowerCase() + ' has been downloaded!',
            type: 'success',
            duration: 5000
        });
...
<template>
    <Menu as="div" class="fixed top-4 right-4 z-50 text-left">
        <div>
            <MenuButton v-if="downloadInProgress"
                        class="cursor-pointer flex items-center gap-2 justify-center px-4 py-2 rounded-full bg-white shadow-md ring-1 ring-gray-300 hover:bg-gray-100">
                <LvProgressBar mode="indeterminate" class="w-6" :value="50" />
                <span class="text-sm font-semibold text-gray-700">{{ downloads.length }}</span>
            </MenuButton>
        </div>

        <transition v-if="downloadInProgress"
            ...
            <MenuItems class="absolute right-0 mt-3 w-80 origin-top-right rounded-xl bg-white shadow-2xl ring-1 ring-black/10 focus:outline-none">
                <div class="py-2 max-h-96 overflow-y-auto">
                    <MenuItem v-for="file in downloads.slice(0,5)" :key="file.content.id" v-slot="{ active }">
                        <div :class="[active ? 'bg-gray-100' : '']" class="flex flex-col gap-2 px-4 py-3 text-sm text-gray-700">
                            <div class="flex items-center gap-3">
                                <div class="flex items-center justify-center w-8 h-8 bg-gray-100 rounded-full">
                                    <Icon v-if="file.content.icon" :name="file.content.icon" class="w-5 h-5 text-gray-500" />
                                    <div v-else class="text-xs text-gray-500">📄</div> <!-- Fallback icon -->
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="font-semibold text-gray-900 text-ellipsis whitespace-nowrap overflow-hidden">
                                        {{ file.content.title }}
                                    </div>
                                </div>
                            </div>
                    ...
                    <MenuItem>
                        <Link :href="route('download-manager')" class=" cursor-pointer flex items-center gap-2 px-4 py-3 text-sm text-gray-700">See all downloads</Link>
                    </MenuItem>
                </div>
            </MenuItems>
        </transition>
